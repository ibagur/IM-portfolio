...
import { MAP_REFERENCE_COORDS } from "../monitoring-area.mjs";
...
assert.equal(grid.length, 25, "the wind layer should contain a 5 by 5 local grid");
assert.ok(grid.some(location => location.latitude === MAP_REFERENCE_COORDS[0] && location.longitude === MAP_REFERENCE_COORDS[1]), "the grid should include an exact El Saler sample");
assert.ok(grid.some(location => location.latitude < MAP_REFERENCE_COORDS[0] && location.longitude < MAP_REFERENCE_COORDS[1]), "the grid should cover the south-western local area");
assert.ok(grid.some(location => location.latitude > MAP_REFERENCE_COORDS[0] && location.longitude > MAP_REFERENCE_COORDS[1]), "the grid should cover the north-eastern local area");
...
assert.throws(() => parseCurrentWindPayload(payload.slice(0, -1), grid), /Expected 25 wind locations/);
