...
detections** over Esri satellite imagery, centred on Onda / Sant Francesc.
It is intended to show detections and their relationship to a supplied area of
interest; it does **not** show a verified wildfire perimeter. Keep that
...
- `monitoring-area.mjs` — shared fixed map centre, zoom, Vall d'Uixó anchor,
  and hotspot monitoring extent.
- `wind-layer.mjs` — pure wind-grid, direction, and Open-Meteo response helpers.
  The browser requests this current-wind layer from Sant Francesc to Vall
  d'Uixó independently on every page load and refreshes it every 10 minutes; it
  must not depend on FIRMS succeeding or changing.
...
- `data/zone-of-interest.kml` — canonical fixed Sant Francesc boundary. Do not
  replace it as part of an ordinary hotspot refresh.
- `data/zone-of-interest.geojson` — browser-ready projection of that KML,
...
- `scripts/export-current-firms.mjs` — fetches Earth Nullschool's packed
  current FIRMS feed, filters the fixed local extent, classifies points against
  the fixed boundary, and atomically rewrites all three generated data files.
...
- `inside_area_of_interest` — boolean calculated with the local KML polygon.

The front end also expects `generated_at`, `source_url`,
`detections_inside_area_of_interest`, and a valid GeoJSON feature collection.
...
For each change, copy the corresponding files (including data files whenever
the local snapshot or boundary changes), validate in the portfolio copy, check
...
- Preserve the fixed-boundary versus refreshable-hotspot split. A normal
  refresh changes `firms-current.geojson`, `current-hotspots-nearby.kml`, and
  the derived `zone-of-interest.geojson`, but not `zone-of-interest.kml`.
...
2. Serve the fixed snapshot and open the map. Confirm the boundary loads, the
...
3. If the exporter changed, run it only when a network refresh is intended;
   then inspect the generated GeoJSON metadata, feature count, coordinates,
   and inside-boundary count. Confirm the KML remains importable.
4. If the workflow changed, ensure its staged file list still excludes the
   canonical `data/zone-of-interest.kml`.
